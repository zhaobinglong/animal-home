...
class JSSDK {
  ...
  public function getAccessToken() {
    // $data = json_encode(file_get_contents("access_token.json"));
    ...
    if ( (int)$data->expire_time < time()) {
      ...
      if ($access_token) {
        ...
        $time = time()+7000;
        $str='{"expire_time":"'.$time.'","access_token":"'.$res->access_token.'"}';
        $end= fwrite($fp, $str);
        $this->log->success('accesstoken更新，本次token是'.$res->access_token);
      }else{
