...
class bike {
	...
	public function uploadExcel() {
		$rws_post = $GLOBALS['HTTP_RAW_POST_DATA'];
		$mypost = json_decode($rws_post);
		$sql = 'insert into market(name,size,price_pre,price_now,price_updown,order_num,remark, scale,create_time) value("' . $mypost->name . '","' . $mypost->size . '","' . $mypost->price_pre . '","' . $mypost->price_now . '","' . $mypost->price_updown . '","' . $mypost->order_num . '", "'.$mypost->remark.'","'.$mypost->scale.'","' . strtotime("today"). '")';
		$res = $this->db->dql($sql);
		$this->sendData($res, $sql);
	}
	...
	public function getMarket() {
		...
		$last_create_time = mysql_fetch_array($this->db->dql($sql), MYSQL_ASSOC)['create_time'];

		$sql = 'select * from market where create_time="'.$last_create_time.'"';
		$res = $this->db->dql($sql);
		$data = array();
		while ($row = mysql_fetch_array($res, MYSQL_ASSOC)) {
			array_push($data, $row);
