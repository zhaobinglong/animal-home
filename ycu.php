...
class ycu {
	...
	public function getCode() {
	    $url = 'http://202.207.232.8/sys/ValidateCode.aspx?t='.mt_rand(1,999999);
	    $header = array(
	        'Content-Type: application/x-www-form-urlencoded',
	        'Host: 202.207.232.8',
	        'Origin: http://202.207.232.8',
	        'Referer: http://202.207.232.8/_data/login_home.aspx'
	    );
	    $res = $this->http->httpGet($url, $header);
	    list($header, $body) = explode("\r\n\r\n", $res);
	    $headers = explode("\r\n", $header); 
		$cookie = explode(": ", $headers[8]); 
	    $name = 'unibbs_ycu_code_valify_'.mt_rand(100,999).'.jpeg';
	...
	public function userLogin($data) {
	    ...
	    $url = 'http://202.207.232.8/_data/login_home.aspx';
	...
	public function getScore($data = null) {
	    $url = 'http://202.207.232.8/xscj/Stu_MyScore_rpt.aspx';
	    $params = array(
	       'sel_xn' => $data['xn'],
	       'btn_search' => '',
	       'SJ' => 1,
	    ...
	    foreach ($html->find('img') as $element) {
	      array_push($imgs, 'http://202.207.232.8/xscj/'.$element->src);
	    }
	    // var_dump($info);
	    // var_dump($imgs);
	    ...
	    for ($i=0; $i < count($imgs); $i++) { 
	        ...
	        $name = 'unibbs_ycu_'.$data['username'].'_'.$data['xn'].'_'.$i.'.jpeg';
	        try {
	          $result = $this->cosClient->putObject(array(
	            'Bucket' => $this->bucket,
	            'Key' => $name,
